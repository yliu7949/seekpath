...
% Get cell parameters
[a, b, c, cosalpha, cosbeta, cosgamma] = utils.getCellParameters(conv_lattice);
...
property = data.getSpacegroupData(spacegroup_number);
bravais_lattice = [property{1}, property{2}];
has_inv = property{3};

% Determine extended Bravais lattice
[ext_bravais, conv_lattice, conv_positions] = hpkot.internal.getExtendedBravaisLattice(bravais_lattice, spacegroup_number, a, b, c, cosalpha, cosbeta, cosgamma, threshold, conv_lattice, conv_positions);
...
[P, invP] = hpkot.internal.getPMatrix(bravais_lattice);

% Get path data
[kparam_def, points_def, path] = hpkot.internal.getPathData(ext_bravais);
...
for i = 1:size(kparam_def, 1)
    ...
    kparam.(kparam_name) = hpkot.internal.evaluateExpression(kparam_expr, a, b, c, cosalpha, cosbeta, cosgamma, kparam);
end

% Extend kparam with additional expressions
kparam_extended = hpkot.internal.extendParameters(kparam);
...
result.reciprocal_primitive_lattice = utils.getReciprocalCellRows(prim_lattice);
